...
<video width="100%" height="auto" controls>
  <source src="/img/nbr-3-open-data.mp4" type="video/mp4">
</video>
...
<video width="100%" height="auto" controls>
  <source src="/img/nbr-4-add-basemap.mp4" type="video/mp4">
</video>
...
<video width="100%" height="auto" controls>
  <source src="/img/nbr-4-order-layers.mp4" type="video/mp4">
</video>

### Step 5: Learn about your data and how to delineate open water features from it
Well, brace yourself: now comes the magic. What happens now? We will process the data so that we can see from the image where burned areas are red, non-burned areas are green.

**How do we do that?** \
We use a method that calculates the different data with a certain formula so that the result is a value between -1 and +1 for each point on the image. -1 stands for "no burned areas" and +1 for, surprise, "burned areas".

This method is called calculating an index. The index is the result of the formula and the formula we use is called NBR, the Normalized Burn Ratio.
...
Cool, we have raster data. But why do you have multiple files of the same location and why do they look different? That’s are different bands. On this project we use Band 8 and Band 12 of the satellite.
...
*Sure, raster data, bands, but what the heck is the NBR?* Great question, my friend. With the specific formula of the NBR we will combine the Bands 8 and 12. Why? Read on:

<details>
  <summary>How does the NBR indicator work?</summary>
    <p>The Normalized Burn Ratio (NBR) is derived from the near and short wave infrared bands. This formula highlights burnt areas in large fire zones.</p>
    <p>Healthy vegetation shows a very high reflectance in the near infrared (NIR), and low reflectance in the short wave infrared (SWIR) portion of the spectrum - the opposite of what is seen in areas devastated by fire. That means, recently burnt areas demonstrate low reflectance in the NIR and high reflectance in the SWIR.</p>
    <p>A high NBR value indicates healthy vegetation while a low value indicates bare ground and recently burnt areas. Non-burnt areas are normally attributed to values close to zero.</p>
</details>

Enough dry theory. Now it's down to the nitty-gritty.

### Step 6: Calculating the NBR
Let’s get ready to rumble. What we will do now, is calculate the NBR from the data you’ve downloaded.

The formula for the NBR is the following: (B08 - B12) / (B08 + B12)

Band 8, or shorter B08, represents the near infrared radiation and Band 12, or B12, the short wave infrared.
...
3. Below the field `Output layer` is a button with 3 points. Click on it and write "NBR" in the file name input field. Click `Save`.
4. Copy the following formular and paste it into the field named `Raster Calculator Expression`: ( "B08@1" - "B12@1" ) / ( "B08@1" + + "B12@1" )
5. Click `Ok`.
6. Wait for it… Voila.

<video width="100%" height="auto" controls>
  <source src="/img/nbr-6-calculate-nbr.mp4" type="video/mp4">
</video>

That’s all. Almost.

### Step 7: Bring colour into the game
To make the data easier to understand, we want that burned areas are red and non-burned areas are green. For that we will colorize the grey image.

1. Double-click the NBR layer in the layers panel. A new window will appear.
...
6. Then choose `All color ramps` and select `RdYlGn`.
7. Click again on the color ramp dropdown and select the first option, `Invert Color Ramp`.
8. Below the table with the colored squares, click on `classify`.
9. Click `Apply` _and_ `Ok`.

<video width="100%" height="auto" controls>
  <source src="/img/nbr-7-colorize-data.mp4" type="video/mp4">
</video>
...
To make your NBR layer more transparent so that you can see the basemap better, you first have to hide the two bands. We don't need them any more. Just click on the tick in front of the layer. My recommendation: Only tick the layers you really need. That way you keep the overview. In our case it is the NBR layer and the basemap:

<video width="100%" height="auto" controls>
  <source src="/img/nbr-8-layer-display.mp4" type="video/mp4">
</video>
...
<video width="100%" height="auto" controls>
  <source src="/img/nbr-8-layer-opacity.mp4" type="video/mp4">
</video>
...
<video width="100%" height="auto" controls>
  <source src="/img/nbr-8-export-map.mp4" type="video/mp4">
</video>